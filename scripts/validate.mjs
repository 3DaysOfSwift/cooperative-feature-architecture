...
export function validate(base=root,{source=true}={}) {
  ...
  for(const name of ['cfa-development','xcode-project-dashboard']) {
  ...
  if(source) {
    ...
    if(hash(path.join(base,'architecture/cfa-specification.md'))!==hash(path.join(base,'skills/cfa-development/references/cfa-specification.md'))) throw Error('Stale architecture copy. Run npm run sync.');
    for(const dir of ['src','Documentation']) for(const file of files(path.join(base,'tools/architecture-dashboard',dir))) {
      if(hash(path.join(base,'tools/architecture-dashboard',dir,file))!==hash(path.join(base,'skills/xcode-project-dashboard/scripts/dashboard',dir,file))) throw Error(`Stale bundled tool: ${file}`);
    }
  }
  return manifest;
}
